...
フロントエンドパッケージのメタデータでは、現在 [frontend/package.json](./frontend/package.json) に MIT License が宣言されています。正式な再利用、再配布、コントリビューションのためには、リポジトリ直下に完全な LICENSE ファイルを追加する必要があります。
