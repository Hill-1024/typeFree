...
- **桌面端体验**: 基于 Electron，支持系统菜单、最近文件、文件打开事件、macOS 文档编辑状态和应用图标。
...
该命令会先启动 Vite 开发服务器，再启动 Electron，并把开发服务器地址注入桌面端。
...
桌面打包由 `electron-builder` 处理，产物输出到：

```text
frontend/release/
```

当前 macOS 配置会生成 `dmg` 和 `zip` 目标。
...
| `pnpm run dev:desktop` | 启动 Electron 桌面开发模式 |
| `pnpm run build` | 构建 Web 版本 |
| `pnpm run build:web` | 构建 Web 版本 |
...
在浏览器环境中，TypeFree 会优先使用 File System Access API；不支持该 API 时会回退到文件选择和下载方式。在桌面端中，Electron 会使用系统原生打开、保存、另存为和重命名能力。
...
- Electron 打包图标: `frontend/build/icon.png`、`frontend/build/icon.icns`
...
| `frontend/electron/` | Electron 主进程、预加载脚本和桌面端国际化 |
| `frontend/scripts/` | 桌面开发和打包脚本 |
