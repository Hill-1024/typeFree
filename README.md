...
TypeFree 是一个开源、自由、本地优先的 Markdown 编辑器。它以纯 Markdown 作为唯一内容源，同时提供接近所见即所得的块级编辑体验：你可以像编辑成品文档一样处理段落、标题、引用、代码、公式和图表，也可以随时切回完整源码模式。

TypeFree 不试图把 Markdown 藏起来。文档仍然是你的本地文件，格式仍然透明，文件仍然可以被任何标准 Markdown 工具读取。编辑器只是让 Markdown 更自然、更少噪音地被书写。
...
- **块级 WYSIWYG 编辑**: 点击任意 Markdown 块即可编辑对应源码，失焦后回到渲染视图。
- **完整源码模式**: 一键切换到全文 Markdown 源码，并在 WYSIWYG 与源码模式之间保持编辑位置同步。
- **本地文件工作流**: 支持新建、打开、保存、另存为、重命名文件，以及关闭前未保存提醒。
- **桌面端体验**: 基于 Tauri，支持系统菜单、最近文件、原生文件对话框、关闭确认和应用图标。
- **数学公式**: 支持行内公式和块级 LaTeX 数学预览。
- **Mermaid 图表**: 支持 Mermaid 图表块预览，并跟随亮色/暗色主题。
- **代码块体验**: 支持语法高亮、行号、语言标识和代码围栏语言补全。
...
仓库根目录是 `pnpm workspace` 入口，会安装 `frontend/` 中的 Web 与 Tauri 前端依赖。
...
| `pnpm run dev:web` | 同上，显式启动 Web 模式 |
| `pnpm run dev:desktop` | 启动 Tauri 桌面开发模式 |
| `pnpm run build` | 构建 Web 版本 |
| `pnpm run build:web` | 同上，显式构建 Web 模式 |
| `pnpm run typecheck` | 运行类型检查 |
| `pnpm run dist:desktop` | 构建并打包桌面应用 |
| `pnpm run preview` | 预览 Web 构建产物 |
...
前端包元数据当前声明为 MIT License，见 [frontend/package.json](./frontend/package.json)。仓库顶层仍需要补充完整的 LICENSE 文件，以便正式复用、分发和贡献时有明确的授权文本。
